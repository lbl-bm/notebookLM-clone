...
export function classifyQuestion(question: string): QuestionType {
  ...
  // 计算匹配分数
  const factualScore = factualKeywords.filter((k) => q.includes(k)).length;
  const summaryScore = summaryKeywords.filter((k) => q.includes(k)).length;
  const analyticalScore = analyticalKeywords.filter((k) => q.includes(k)).length;
  const comparativeScore = comparativeKeywords.filter((k) =>
    q.includes(k),
  ).length;

  // 选择分数最高的类型
  // 使用优先级权重打破平局：summary > analytical > comparative > factual > general
  // 原因：更宽泛的问题类型 prompt 对错误匹配的容忍度更高，而 factual 最窄
  ...
  // 平局时按优先级决定：summary > analytical > comparative > factual
  const candidates: Array<[number, QuestionType]> = [
    [summaryScore, QuestionType.SUMMARY],
    [analyticalScore, QuestionType.ANALYTICAL],
    [comparativeScore, QuestionType.COMPARATIVE],
    [factualScore, QuestionType.FACTUAL],
  ];

  for (const [score, type] of candidates) {
...
/**
 * 按 token 预算从历史消息中选取尽可能多的最近消息
 * 从最新消息往前选，保证不超过 maxTokens
 */
export function selectHistoryByTokenBudget(
  ...
  let usedTokens = 0;
  const selected: typeof chatHistory = [];
...
/**
 * 组装完整的消息列表
 * 支持动态 Prompt 选择 + M1 证据约束 + token 预算控制历史长度
 */
export function buildMessages(params: {
  ...
  /**
   * 历史消息最大 token 预算（默认 2000）
   * 超出时从最旧的消息开始丢弃，保留最近的对话
   */
  ...
  // 按 token 预算选取历史消息（替换原来固定 6 条的方式）
  const selectedHistory = selectHistoryByTokenBudget(chatHistory, historyTokenBudget);
