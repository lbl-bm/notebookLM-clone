...
## 更新摘要
**变更内容**
- 重构中间件认证逻辑，实现更精确的路由匹配和条件认证检查
- 新增 `protectedPrefixes` 数组和辅助函数 `needsAuth()`、`isAuthRoute()`
- 增强中间件匹配器以排除 API 路由，提升性能
- 优化认证流程，仅对需要认证的路由调用 auth API
...
**图表来源**
- [middleware.ts](file://middleware.ts#L1-L101)
...
- [middleware.ts](file://middleware.ts#L30-L86)
...
### 中间件：路由保护与会话刷新
- **重构优化**：使用 `protectedPrefixes` 数组和辅助函数 `needsAuth()`、`isAuthRoute()` 实现精确路由匹配
- **性能提升**：仅对需要认证的路由调用 `getUser()`，而非检查每个路由
- **API 路由排除**：增强匹配器排除 API 路由，允许 API 自行处理认证验证
- **条件认证检查**：通过 `needsAuthCheck` 和 `isAuth` 快速路径优化中间件执行

```mermaid
flowchart TD
Enter(["进入中间件"]) --> ExtractPath["提取 pathname"]
ExtractPath --> CheckAuth{"needsAuth(pathname)?"}
ExtractPath --> CheckAuthRoute{"isAuthRoute(pathname)?"}
CheckAuth --> |否且| CheckAuthRoute
CheckAuth --> |是| CheckAuth
CheckAuthRoute --> |否且| CheckAuth
CheckAuthRoute --> |是| CheckAuthRoute
CheckAuth --> |否且| CheckAuthRoute
CheckAuth --> |是| FastPath["快速放行"]
CheckAuthRoute --> |是| FastPath
FastPath --> End(["结束"])
CheckAuth --> |是| GetUser["getUser() 刷新会话"]
CheckAuthRoute --> |是| GetUser
GetUser --> Path{"路径匹配"}
...
**图表来源**
- [middleware.ts](file://middleware.ts#L20-L86)
...
- [middleware.ts](file://middleware.ts#L47-L63)
...
## 性能考量
- **中间件优化**：重构后仅对需要认证的路由调用 `getUser()`，显著减少不必要的会话刷新调用
- **快速路径优化**：通过 `needsAuth()` 和 `isAuthRoute()` 辅助函数实现快速路径，避免对每个路由都进行复杂检查
- **API 路由排除**：中间件匹配器排除 API 路由，允许 API 自行处理认证验证，减少中间件开销
- OAuth 回调仅在授权完成后触发，避免频繁网络往返
- 权限校验工具应尽量在服务端执行，避免客户端侧重复校验
- 大量查询可利用数据库索引与分页，降低响应时间

**章节来源**
- [middleware.ts](file://middleware.ts#L14-L28)
- [middleware.ts](file://middleware.ts#L30-L86)
...
- 403 访问被拒绝
  ...
  - 检查权限校验工具的调用位置与时机
- 中间件重定向异常
  - 检查 `protectedPrefixes` 和 `authRoutes` 数组配置
  - 确认 `needsAuth()` 和 `isAuthRoute()` 辅助函数的路由匹配逻辑
  - 验证中间件匹配器是否正确排除了 API 路由
- 性能问题
  - 确认中间件是否正确使用了快速路径优化
  - 检查是否有不必要的路由被中间件拦截
...
- [middleware.ts](file://middleware.ts#L88-L100)

## 结论
本项目采用 Supabase Auth 作为统一认证与会话管理基础，结合重构后的中间件实现精确的路由级保护，配合服务端权限校验工具实现资源级所有权控制。客户端与服务端分别封装了对应的 Supabase 客户端以满足不同场景需求。新的中间件架构通过 `protectedPrefixes` 数组和辅助函数实现了更高效的路由匹配，性能得到显著提升，同时保持了良好的扩展性与安全性基础。
...
### 中间件重构详解
**更新** 介绍了 middleware.ts 的重大重构，实现了更精确的路由匹配和条件认证检查

#### 新的中间件架构特点
- **精确路由匹配**：使用 `protectedPrefixes` 数组定义受保护路由前缀
- **辅助函数优化**：`needsAuth()` 和 `isAuthRoute()` 提供清晰的路由分类逻辑
- **性能优化**：快速路径检查，避免对每个路由都进行复杂认证检查
- **API 路由排除**：中间件匹配器排除 API 路由，允许 API 自行处理认证

#### 关键实现细节
- `protectedPrefixes`：定义需要认证的路由前缀数组
- `authRoutes`：定义已登录用户不应访问的路由
- `needsAuth()`：检查路径是否需要认证
- `isAuthRoute()`：检查路径是否是认证路由
- 增强的 `config.matcher`：排除静态资源和 API 路由
