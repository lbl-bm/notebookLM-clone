...
- 检索器：对外暴露向量检索与混合检索，聚合来源元数据，返回带证据的检索结果。
...
## 架构总览
下图展示从“用户输入/知识源”到“向量检索”的端到端流程，强调嵌入维度一致性、批量处理与重试机制、混合检索与索引优化。
...
## 结论
本系统以“强约束 + 批量优化 + 混合检索”为核心设计，通过严格的嵌入维度一致性、完善的重试与日志体系、以及 HNSW 索引与混合检索，实现了稳定高效的向量检索能力。建议在生产环境中持续关注维度配置、索引维护与日志分析，以保障检索质量与性能。
